...
def loaddata(f):
    f = h5py.File(f,'r')
    data = {ch:esc.Array.load_from_h5(f, f['channels'][ch][()]) for ch in f['channels'].keys()}
    return data
...
def analyse_filter(runno, filters={'i0_sum':[0,700]}, sig = ['jf_pk'], i0 = ['jf_fl'], plot_hist = False, plot = True, save=False, noimg=True):
    ...
    if plot:
        ...
        for n, ((jf, jfd), (jf_filt, jfd_filt)) in enumerate(zip(jfs.items(), jfs_filt.items())):
            ...
            ax[n][0].plot(xset, (jfd['on']/datas_av[i0ch]['on'])/(jfd['off']/datas_av[i0ch]['off']), 'g', label=f'{jf} ratio')
            ax[n][0].plot(xset_filt, (jfd_filt['on']/datas_filt_av[i0ch]['on'])/(jfd_filt['off']/datas_filt_av[i0ch]['off']), 'b', label=f'{jf_filt} filtered ratio')
            ...
            ax[n][1].plot(xset, (jfd['on']/datas_av[i0ch]['on']), 'g', label=f'{jf} on')
            ax[n][1].plot(xset, (jfd['off']/datas_av[i0ch]['off']), 'orange', label=f'{jf} off')
...
def apply_filter(runno, filters={'i0_sum':[0,700]}, sig = ['jf_pk'], plot_hist = False, noimg=False, tt=False, pid_offset=0):
    ...
    for las in ['on', 'off']:
        ...
        for filt_ch, filt_th in filters.items():
            if filt_th == 'auto':
                filt_std = np.std( datas[filt_ch][las][(datas[filt_ch][las].data-np.median(datas[filt_ch][las].data))<2*np.std(datas[filt_ch][las].data)])
                ...
                print(f'Auto set {filt_ch} threshold to [{filt_th[0]:.3},{filt_th[1]:.3}]')
            mask.append((datas[filt_ch][las].data > filt_th[0]) * (datas[filt_ch][las].data < filt_th[1]))
        mm = mask[0]
        for m in mask:
            mm = mm&m
        filt[las]=mm
    ...
    if plot_hist:
        ...
        for n, (k, th) in enumerate(filters.items()):
            ...
            ax[n][1].plot(datas[k]['off'].data[:2000], datas[sig[0]]['off'].data[:2000], '.')
            ax[n][1].plot((datas[k]['off'][:2000])[mask[n][:2000]].data, (datas[sig[0]]['off'][:2000])[mask[n][:2000]].data, '.', label = 'filter')
            ax[n][1].plot((datas[k]['off'][:2000])[filt['off'][:2000]].data, (datas[sig[0]]['off'][:2000])[filt['off'][:2000]].data, '.', label = 'all_filter')
...
def analyse_filter_timetool(
    ...
    pos_all = np.concatenate([(np.zeros((step_length,))+step_pos) for step_length , step_pos in zip(datas_filt['tt_sig']['on'].scan.step_lengths, xset)]).T
    first_jf = [key for key in datas_filt.keys() if 'jf' in key][0]
    ...
    if not overwrite_tt and os.path.exists(tt_filename):
        ...
        edge_pos_ea, edge_amp_ea = timetool.analyse_edge_correlation(datas['tt_sig'])
    ...
    filt_ea['on'], edge_pos_ea, edge_amp_ea = esc.match_arrays(filt_ea['on'], edge_pos_ea, edge_amp_ea)
    ...
    print(f'Rejecting {(len(tt_filt)-sum(tt_filt))/len(tt_filt)*100}% of shots because of bad tt results')
    ...
    if sub_median:
        ...
        print(f'difference between median and fixed value: {np.median(edge_pos_ea.data)-tt_offset}')
...
def phiScan(runno, save=True, plot=False):
    ...
    if plot:
        ...
        ax[1].plot(pos,(pk_av['on'] - bg_av['on'])/fl_av['on']/((pk_av['off'] - bg_av['off'])/fl_av['off']))
        ax[1].set_xlabel("Phi")
        #fig.tight_layout()
...
#    plt.pcolor(t[10:600],freq[4:25],td_fft[0][4:25,10:600],vmin=-5,vmax=5)
    if plot is True:
        ...
        plt.imshow(z, cmap='jet',
               ...
               interpolation='spline36', origin='lower',clim=(z.min(),clm),aspect='auto')
...
def gaussian(t,a0,t0,sig,bg):
    return bg+(a0/np.sqrt(2*np.pi)/sig*np.exp(-(t-t0)**2/2/sig**2))
