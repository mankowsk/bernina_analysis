...
class TtProcessor:
    ...
    def save_files(self):
        ...
        np.save(
            ...
            [
                self.corr_pos_av,
                self.corr_pos_av_std,
                self.corr_amp_av,
                self.corr_amp_av_std,
            ],
        )
        np.save(
            file_name2,
            [np.hstack(self.pid), np.hstack(self.corr_pos), np.hstack(self.corr_amp)],
        )
    ...
    def analyse_edge_correlation_noea(
        self, tt_sig, ids, edge=None, roi=[650, 1350], smooth=80
    ):
        tt_sig["off_sm"] = scipy.ndimage.uniform_filter(
            tt_sig["off"], size=(10, smooth)
        )
    ...
    def take_pumped_background(self):
        ...
        self.roi = [
            len(self.tt_pumped) - find_fall(abs(self.tt_pumped[::-1]), 0.2)[0],
            find_fall(abs(self.tt_pumped), 0.2)[0],
        ]
    ...
    def evaluate(self):
        ...
        tt_sig, ids = on_off(
            [self.tt_sig[:, self.roi[0] : self.roi[1]], self.ids], self.evts
        )
        if len(ids["off"]) == 0:
        ...
        if self.edge is None:
            if self.step_type == "data":
                ...
                self.edge_roi = [
                    int(pts / 2 - self.step_width / 2),
                    int(pts / 2 + self.step_width / 2),
                ]
                if self.direction == "falling":
                    self.edge = -self.edge
                corr_pos, corr_amp, tt_ratio_sm = self.analyse_edge_correlation_noea(
                    tt_sig, ids, edge=self.edge, roi=self.edge_roi, smooth=self.smooth
                )
        ...
        if self.memory <= self.counter_glob:
            self.pid_2 = ids["on"][-1]
            self.counter_glob = 0
    ...
    def setup_plot(self):
        ...
        self.fig, self.axs = plt.subplots(
            4, 2, num=f"BSEN drift monitor cam {self.cam}", figsize=(7, 12)
        )
        ...
        self.axs[0][0].plot(
            np.asarray(self.corr_pos_av), color="royalblue", label="edge pos (px)"
        )
        self.axs[0][0].plot(
            np.asarray(self.corr_pos_av) + np.asarray(self.corr_pos_av_std),
            color="royalblue",
            alpha=0.3,
            label=f"+/- std",
        )
        ...
        self.axs[0][1].plot(
            np.asarray(self.corr_amp_av), color="seagreen", label="edge amp"
        )
        self.axs[0][1].plot(
            np.asarray(self.corr_amp_av) + np.asarray(self.corr_amp_av_std),
            color="seagreen",
            alpha=0.3,
            label=f"+/- std",
        )
        ...
        self.axs[1][1].axvline(
            self.edge_roi[0], color="black", linestyle="dashed", linewidth=1
        )
        self.axs[1][1].axvline(
            self.edge_roi[1], color="black", linestyle="dashed", linewidth=1
        )
        self.axs[1][0].plot(self.tt_ratio_sm[-1] + 1, color="royalblue")
        self.axs[1][0].axvline(
            self.corr_pos[-1][-1] + 1, color="red", linestyle="dashed", linewidth=1
        )
        ...
        freq, amp, hist, bins, fit, fit_values = self.diag_hist_fft(
            self.corr_amp, nbins=100
        )
    ...
    def update_plot(self, frame):
        ...
        self.update_ax_data(
            self.axs[0][0], [0, 1, 2], [x, x, x], [y, y + ystd, y - ystd]
        )
        ...
        if not self.smallplot:
            ...
            self.update_ax_data(
                self.axs[0][1], [0, 1, 2], [x, x, x], [y, y + ystd, y - ystd]
            )
            ...
            self.update_ax_data(
                ...
                labels=[
                    "data",
                    f'fwhm {fit_values["sigma"]*2.3548:.4} cen {int(fit_values["center"])}',
                ],
            )
            freq, amp, hist, bins, fit, fit_values = self.diag_hist_fft(
                self.corr_amp, nbins=100
    ...
    def plot_animation(self, name="TT online ana", animate=True):
        ...
        if animate:
            self.ani = FuncAnimation(
                self.fig, self.update_plot, interval=self.Nshots * 10
            )
            # plt.show()
